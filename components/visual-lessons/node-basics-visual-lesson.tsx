...
export function NodeBasicsVisualLesson({ referenceContent }: { referenceContent?: ReactNode }) {
  ...
  function start() { setStarted(true); setStep(0); setPlaying(true); }
  function next() { setStarted(true); setPlaying(false); setStep((current) => Math.min(current + 1, LAST_STEP)); }
  function replay() { setStarted(true); setStep(0); setPlaying(true); }

  return (
    <section className="visualLesson visualJourney" aria-labelledby="visual-lesson-title">
      <div className="visualLessonTopbar">
        <div><span className="visualLive"><i /> VISUAL LESSON</span><span className="visualLessonMeta">رحلة 01 · اللغة وبيئة التشغيل</span></div>
      ...
      {detailsOpen ? (
        <div className="visualDetailsPanel">
          <div className="visualDetailsHeader">
            <div><span>التفاصيل والمرجع</span><strong>الفصل 01 · الشرح الكامل</strong></div>
            <button onClick={() => setDetailsOpen(false)}><ArrowRight size={16} /> العودة للمشهد</button>
          </div>
      ...
      <div className="visualLessonControls">
        ...
        <div className="visualLessonActions">
          <button className="visualReferenceButton" onClick={() => setDetailsOpen((value) => !value)}>
            <BookOpenText size={16} />{detailsOpen ? "العودة للمشهد" : "التفاصيل والمرجع"}<ArrowDown size={15} />
          </button>
        </div>
      </div>
    </section>
